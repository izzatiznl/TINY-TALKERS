# LEARNING-APPLICATION-USING-MIT-APP-INVENTOR
Tiny Talkers – An educational MIT App Inventor app for kids to learn English vocabulary, colors, numbers, pronunciation, and interactive quizzes.


# Tiny Talkers

**Tiny Talkers** is an interactive educational app designed for children to learn basic English vocabulary in a fun and engaging way. Developed using **MIT App Inventor**, the app combines visual learning, voice pronunciation, and interactive quizzes to help kids improve their language skills.

## Key Features
- **Learn Basic Vocabulary:** Colors, numbers, fruits, and everyday objects.
- **Voice Pronunciation:** Kids can tap to hear how words are pronounced.
- **Interactive Quiz:** Fun quizzes to reinforce learning.
- **Shake Interaction:** Shake the device to change images into number characters for vocabulary practice.
- **User-Friendly Interface:** Colorful and easy-to-navigate screens suitable for young learners.

How to Open the Project:
A)Import Project in MIT App Inventor
1.Open the MIT App Inventor website: MIT App Inventor
2.Sign in using a Google account.
3.Click Create Apps.
4.Select Projects → Import project (.aia) from my computer.
5.Choose the provided .aia file.
6.Wait for the project to load.

B)Test on Android Device
1.Install the official MIT AI2 Companion App on your Android device.
2.In MIT App Inventor, click Connect → AI Companion.
3.Scan the QR code displayed on the screen.
The application will run directly on your device.
